...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		void GetTarget()
		{
			if(nearAxis == Axis.None && (Input.GetMouseButtonDown(0) || m_SelectAction.GetStateDown(m_TargetSource) ))
			{
				...
				RaycastHit hitInfo;
                //Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask);


                if (Physics.Raycast(m_RaycastSource.position, m_RaycastSource.TransformDirection(Vector3.forward), out hitInfo, Mathf.Infinity, selectionMask))
				{
